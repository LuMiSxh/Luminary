...
type ChapterMetadata struct {
	...
	ProviderID   string   // ID of the provider (for uniqueness)
}
...
func (d *DownloadService) DownloadChapter(ctx context.Context, config DownloadJobConfig) error {
	...
	// Get concurrency from context or use default
	concurrency := core.GetConcurrency(ctx, 5)
	if d.Logger != nil {
		d.Logger.Debug("Using concurrency limit of %d from context", concurrency)
	}

	// Check for volume override in context
	if volumeOverride, hasOverride := core.GetVolumeOverride(ctx); hasOverride {
	...
	if config.Metadata.MangaTitle != "" {
		...
		mangaTitle = fmt.Sprintf("%s-%s", config.Metadata.ProviderID, config.Metadata.MangaID)
		if mangaTitle == "-" {

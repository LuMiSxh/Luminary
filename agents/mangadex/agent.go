...
// State holds MangaDex-specific data
type State struct {
	ServerNetwork []string
}

// Agent is the main struct for the MangaDex agent
type Agent struct {
	*api.APIAgent
	state  *State
...
func (m *Agent) Initialize(ctx context.Context) error {
	// Call the parent Initialize method
	if err := m.APIAgent.Initialize(ctx); err != nil {
		return err
	}
...
// NewAgent creates a new MangaDex agent
func NewAgent(e *engine.Engine) engine.Agent {
	...
	baseAgent := api.NewAPIAgent(e, config)

	// Create our custom agent
	agent := &Agent{
		APIAgent: baseAgent.(*api.APIAgent),
		state:    state,
		engine:   e,
	}
...
func processChapters(state *State) api.ProcessChaptersFunc {
	return func(ctx context.Context, agent *api.APIAgent, response interface{}, mangaID string) ([]engine.ChapterInfo, bool, error) {
		...
		if len(chaptersResp.Data) == 0 {
			return []engine.ChapterInfo{}, false, nil
		}
...
func createChapterProcessor(e *engine.Engine, state *State) api.ResponseProcessor {
	return func(response interface{}, chapterID string) (interface{}, error) {
		...
		pagesResp, ok := pagesResponse.(*PagesResponse)
		if !ok {
...
func configureExtractors() map[string]engine.ExtractorSet {
	...
	// Chapter extractor
	extractorSets["chapter"] = engine.ExtractorSet{
